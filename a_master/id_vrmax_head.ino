...
void poll_vrmax_coll_head_extension(){
  //Serial.println("poll");
  uint8_t but0,but1,but2;
  uint8_t e[7];
...
void set_button_mode_and_radio_switch_aware (byte i,bool val,byte dir){
 ...
 // turn direction - 0 left, 1 right
  ...
  if ((i != 6) && (i != 4) && (i != 5)){ // radio panel
    if (g_radio_mode== 1){// middle position of panel sw
      if (g_radio_device== 0){ // COM/NAV 1
        if (dir == 0){
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r0l - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r0l - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r0r - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r0r - 1);
        }
      } else if (g_radio_device== 1) { // COM/NAV 2
        //Serial.println("set NAV2");
        if (dir == 0) {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r1l - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r1l - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r1r - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r1r - 1);
        }
      } else if (g_radio_device== 2) {// XPDR
        //Serial.println("set XPDR");
        if (g_nav_mode!= 1){
          if (dir == 0){
            j_vrmpnl.setButton(radio_panel_knob_matrix[i].r2l - 1,val);
            e_state[i].button_id = (radio_panel_knob_matrix[i].r2l - 1);
          } else {
            j_vrmpnl.setButton(radio_panel_knob_matrix[i].r2r - 1,val);
            e_state[i].button_id = (radio_panel_knob_matrix[i].r2r - 1);
          }
        } else {
          if (dir == 0){
            j_vrmpnl.setButton(radio_panel_knob_matrix[i].adfl - 1,val);
            e_state[i].button_id = (radio_panel_knob_matrix[i].adfl - 1);
          } else {
            j_vrmpnl.setButton(radio_panel_knob_matrix[i].adfr - 1,val);
            e_state[i].button_id = (radio_panel_knob_matrix[i].adfr - 1);
          }
        }
      }
    } else if (g_radio_mode== 0) {
      if (g_radio_device== 0) {// PANEL MODE SW UP + STACK SELECTOR SW UP
        if (dir == 0){
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r0lpm1 - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r0lpm1 - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r0rpm1 - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r0rpm1 - 1);
        }
      } else if (g_radio_device== 1) {// PANEL MODE SW UP + STACK SELECTOR SW MIDDLE
        if (dir == 0) {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].m0l - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].m0l - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].m0r - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].m0r - 1);
        }
      } else if (g_radio_device== 2) {// PANEL MODE SW UP + STACK SELECTOR SW DOWN
        if (dir == 0){
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r2lpm1 - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r2lpm1 - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r2rpm1 - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r2rpm1 - 1);
        }       
      }
    } else if (g_radio_mode== 2) {
      if (g_radio_device== 0) {// PANEL MODE SW DOWN + STACK SELECTOR SW UP
        if (dir == 0) {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r0lpm2 - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r0lpm2 - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r0rpm2 - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r0rpm2 - 1);
        }
      } else if (g_radio_device== 1) { // PANEL MODE SW DOWN + STACK SELECTOR SW MIDDLE
        if (dir == 0) {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].m2l - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].m2l - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].m2r - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].m2r - 1);
        }
      } else if (g_radio_device== 2) { // PANEL MODE SW DOWN + STACK SELECTOR SW DOWN
        if (dir == 0) {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r2lpm2 - 1,val);
          e_state[i].button_id = (radio_panel_knob_matrix[i].r2lpm2 - 1);
        } else {
          j_vrmpnl.setButton(radio_panel_knob_matrix[i].r2rpm2 - 1,val);
    ...
    if (i == 5){
      if (g_alt_mode== 0) { // BARO
        if (dir == 0) {
          j_vrmpnl.setButton(alt_knob_conf[0].a0l - 1,val);
          e_state[i].button_id = (alt_knob_conf[0].a0l - 1);
        } else {
          j_vrmpnl.setButton(alt_knob_conf[0].a0r - 1,val);
          e_state[i].button_id = (alt_knob_conf[0].a0r - 1);
        }
      } else if (g_alt_mode== 2) { // RADIO
        if (dir == 0){
          j_vrmpnl.setButton(alt_knob_conf[0].a2l - 1,val);
          e_state[i].button_id = (alt_knob_conf[0].a2l - 1);
        } else {
          j_vrmpnl.setButton(alt_knob_conf[0].a2r - 1,val);
          e_state[i].button_id = (alt_knob_conf[0].a2r - 1);
        }
      } else {
        if (dir == 0) { // just in case you need them
          j_vrmpnl.setButton(alt_knob_conf[0].a1l - 1,val);
          e_state[i].button_id = (alt_knob_conf[0].a1l - 1);
        } else {
          j_vrmpnl.setButton(alt_knob_conf[0].a1r - 1,val);
          e_state[i].button_id = (alt_knob_conf[0].a1r - 1);
        }
      }
    } else if (i == 4) {
      if (g_nav_mode== 0) {
        //j_vrmpnl.setButton(45 + i + offset, val);
        //e_state[i].button_id = (45 + i + offset);
        if (dir == 0){
          j_vrmpnl.setButton(nav_knob_conf[0].n0l - 1,val);
          e_state[i].button_id = (nav_knob_conf[0].n0l - 1);
        } else {
          j_vrmpnl.setButton(nav_knob_conf[0].n0r - 1,val);
          e_state[i].button_id = (nav_knob_conf[0].n0r - 1);
        }
      } else if (g_nav_mode== 1) {
       // j_vrmpnl.setButton(49 + i + offset, val);
       // e_state[i].button_id = (49 + i + offset);
       if (dir == 0) {
          j_vrmpnl.setButton(nav_knob_conf[0].n1l - 1,val);
          e_state[i].button_id = (nav_knob_conf[0].n1l - 1);
        } else {
          j_vrmpnl.setButton(nav_knob_conf[0].n1r - 1,val);
          e_state[i].button_id = (nav_knob_conf[0].n1r - 1);
        }
      } else if (g_nav_mode== 2) {
       // j_vrmpnl.setButton(52 + i + offset, val);
       // e_state[i].button_id = (52 + i + offset);
       if (dir == 0) {
          j_vrmpnl.setButton(nav_knob_conf[0].n2l - 1,val);
          e_state[i].button_id = (nav_knob_conf[0].n2l - 1);
        } else {
          j_vrmpnl.setButton(nav_knob_conf[0].n2r - 1,val);
          e_state[i].button_id = (nav_knob_conf[0].n2r - 1);
        }
      }
    } else { // MAG knob has 1 mode
      //j_vrmpnl.setButton(41 + i + offset, val);
      //e_state[i].button_id = (41 + i + offset);
      if (dir == 0) {
        j_vrmpnl.setButton(mag_knob_conf[0].l - 1,val);
        e_state[i].button_id = (mag_knob_conf[0].l - 1);
      } else {
        j_vrmpnl.setButton(mag_knob_conf[0].r - 1,val);
...
void parse_radio_panel_switches (byte b, byte start_pos) {
  ...
  for (byte i = start_pos; i < end_pos; i++) {
    ...
    if ((g_radio0_unchecked == 1) && (g_radio2_unchecked== 1)) {
      g_radio_device= 1;
    }
    if ((g_radio_mode0_unchecked == 1) && (g_radio_mode2_unchecked == 1)) {
      g_radio_mode= 1;
    }
    if ((g_alt_mode0_unchecked == 1) && (g_alt_mode2_unchecked == 1)) {
      g_alt_mode= 1;
    }
    if ((g_nav_mode0_unchecked == 1) && (g_nav_mode2_unchecked == 1)) {
      g_nav_mode= 1;
    }
    ...
    if (i == RADIO_DEVICE_SWITCH_XPDR_POSITION_JOY_BUTTON - 1) {
      if (v == 1) {
        g_radio_device= 2;
        g_radio2_unchecked= 0;
      } else {
        g_radio2_unchecked= 1;
      }
    } else if (i == RADIO_DEVICE_SWITCH_XPDR_POSITION_JOY_BUTTON) {
      if (v == 1) {
        g_radio_device= 0;
        g_radio0_unchecked= 0;
      } else {
        g_radio0_unchecked = 1;
      }
    }
    else if (i == RADIO_MODE_SWITCH_LOWER_POSITION_JOY_BUTTON - 1) {
      if (v == 1) {
        g_radio_mode= 2;
        g_radio_mode2_unchecked = 0;
      } else {
        g_radio_mode2_unchecked = 1;
      }
    } else if (i == RADIO_MODE_SWITCH_LOWER_POSITION_JOY_BUTTON) {
      if (v == 1) {
        g_radio_mode= 0;
        g_radio_mode0_unchecked = 0;
      } else {
        g_radio_mode0_unchecked = 1;
      }
    } else if (i == ALTIMETER_SELECTOR_JOY_BUTTON - 1) {
      if (v == 1) {
        g_alt_mode= 2;
        g_alt_mode2_unchecked = 0;
      } else {
        g_alt_mode2_unchecked = 1;
      }
    } else if (i == ALTIMETER_SELECTOR_JOY_BUTTON) {
      if (v == 1) {
        g_alt_mode= 0;
        g_alt_mode0_unchecked = 0;
      } else {
        g_alt_mode0_unchecked = 1;
      }
    } else if (i == NAV_MODE_SELECTOR_JOY_BUTTON - 1) {
      if (v == 1) {
        g_nav_mode= 2;
        g_nav_mode2_unchecked = 0;
      } else {
        g_nav_mode2_unchecked = 1;
      }
    } else if (i == NAV_MODE_SELECTOR_JOY_BUTTON) {
      if (v == 1) {
        g_nav_mode= 0;
        g_nav_mode0_unchecked = 0;
      } else {
        g_nav_mode0_unchecked = 1;
      }
    } else if (i == OBS_RATE_SELECTOR_JOY_BUTTON - 1) {
      if (v != radio_matrix[i].sw_val) {
        if (v == 1) {
         //Serial.println(g_obs_rate);
         g_obs_rate = !g_obs_rate;
      ...
      if (v != radio_matrix[i].sw_val) {
        if (g_radio_device== 0) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].c1f, v);
          radio_matrix[i].sw_val = v;
        } else if (g_radio_device== 1) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].c2f, v);
          radio_matrix[i].sw_val = v;
        } 
        if (g_radio_device== 2) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].x1, v);
          radio_matrix[i].sw_val = v;
        }
      }
      ...
      if (v != radio_matrix[i].sw_val) {
        if (g_radio_device== 0) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].c1c, v);
          radio_matrix[i].sw_val = v;
        } else if (g_radio_device== 1) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].c2c, v);
          radio_matrix[i].sw_val = v;
        } 
        if (g_radio_device== 2) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].x2, v);
          radio_matrix[i].sw_val = v;
        }
      }
      ...
      if (v != radio_matrix[i].sw_val) {
        if (g_radio_device== 0) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].n1f, v);
          radio_matrix[i].sw_val = v;
        } else if (g_radio_device== 1) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].n2f, v);
          radio_matrix[i].sw_val = v;
        } 
        if (g_radio_device== 2) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].x3, v);
          radio_matrix[i].sw_val = v;
        }
      }
      ...
      if (v != radio_matrix[i].sw_val) {
        if (g_radio_device== 0) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].n1c, v);
          radio_matrix[i].sw_val = v;
        }
        else if (g_radio_device== 1) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].n2c, v);
          radio_matrix[i].sw_val = v;
        } 
        if (g_radio_device== 2) {
          j_vrmpnl.setButton(radio_panel_pb_matrix[0].x4, v);
          radio_matrix[i].sw_val = v;
        }
      }
    } else 
    ...
    if (i == XPDR_MODE_SW_JOY_BUTTON - 1) {
      if (v == 1) {
        g_xpdr_mode2_unchecked = 0;
      } else {
        g_xpdr_mode2_unchecked = 1;
      }
    } else if (i == XPDR_MODE_SW_JOY_BUTTON) {
      if (v == 1) {
        g_xpdr_mode0_unchecked = 0;
      } else {
        g_xpdr_mode0_unchecked = 1;
      }
    }
      //j_vrmpnl.setButton(first_joy_b + i, v);
  }
}
