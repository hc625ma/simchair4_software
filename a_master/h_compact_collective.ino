...
//    Serial.println(ms); // <- uncomment to see decimal vals for mode switch positions
  ...
  void set_thr_latch_state(uint16_t raw_thr) {
    ...
    if ((raw_thr > COMPACT_COLLECTIVE_IDLE_DETENT_AXIS_VAL) && (g_idle_rel_btn_pressed == 0)) {
  ...
  void apply_advanced_throttle_features (uint16_t raw_thr){
    ...
    if (BUTTON_PRESS_ON_THROTTLE_CUTOFF == 1) {
      uint16_t diff;
      if (raw_thr > COMPACT_COLLECTIVE_THR_MIN) {
        diff = raw_thr - COMPACT_COLLECTIVE_THR_MIN;
      } else {
        diff = COMPACT_COLLECTIVE_THR_MIN - raw_thr;
      }
